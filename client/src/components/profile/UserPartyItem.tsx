...
export default function UserPartyItem({ room, color }: { room: UserPartyRoom, color: string }): JSX.Element {
    ...
    return (
        <div className={styles.user__party__item}>
            <div className={styles.user__party__item__header}>
                <span>{room.Party.time} | {room.Party.date}</span>
            ...
            <div style={{ backgroundColor: color }} className={styles.user__party__item__footer}>
                <span onClick={() => nav(`/chat/${room.id}`)}>Перейти в чат {room.title}</span>
            </div>
        </div>
    )
}
